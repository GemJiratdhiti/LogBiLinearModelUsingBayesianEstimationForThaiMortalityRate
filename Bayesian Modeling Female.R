...
ABmle <- read.csv("Female data/output_female.csv")
Ktmle <- read.csv("Female data/output_kt_female.csv")
Dxt <- as.matrix(read.csv("Female data/Mortality Female.csv"))[,-1]
Ext <- as.matrix(read.csv("Female data/Population Female.csv"))[,-1]
...
par(mfrow = c(1,1))
plot(x = 0:(M - 1), y = PostmeanA, main = "Estimated Alpha (F)",
...
plot(x = 0:(M - 1), y = PostmeanB, main = "Estimated Beta (F)",
     xlab = "Age", ylab = "Beta", type = "b", col = "red", pch = 4)
lines(x = 0:(M - 1), y = beta.mle, type = "b", col = "blue", pch = 3)
legend(x = 72, y = 0.072, legend = c("MLE","Bayes"), col = c("blue","red"),
       lty = c(3,4), pch = c(3,4), cex = 0.7)
plot(x = 2540:2559, y = PostmeanK, main = "Estimated Kappa (F)",
     xlab = "Year", ylab = "Kappa", type = "b", col = "red", pch = 4)
lines(x = 2540:2559, y =  kappa.mle, type = "b", col = "blue", pch = 3)
legend(x = 2553.5, y = 16, legend = c("MLE","Bayes"), col = c("blue","red"),
       lty = c(3,4), pch = c(3,4), cex = 0.7)
PostmeanA - alpha.mle
PostmeanB - beta.mle
PostmeanK - kappa.mle
...
plot.logm.mle <- function(a, b){
  plot(x = 0:(M - 1), y = logm.mle(b - 2539), main = "Female log mortal rate (MLE)",
...
plot.logm.bayes <- function(a, b){
  plot(x = 0:(M - 1), y = logm.bayes(b - 2539), main = "Female log mortal rate (Bayes)",
       xlab = "Age", ylab = "Log mortality rate", type = "l", col = "red")
  for(i in (b - a):1){
...
write.csv(PostmeanA, "output_axbay_female.csv", row.names = F)
write.csv(PostmeanB, "output_bxbay_female.csv", row.names = F)
write.csv(PostmeanK, "output_ktbay_female.csv", row.names = F)